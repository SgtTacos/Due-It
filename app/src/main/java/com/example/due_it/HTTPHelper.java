...
public class HTTPHelper {
    ...
    public static String readHTTP(String url, String authorization) {
        try {
             ...
             * I prefer the BufferedReader since it provides
             * the readLine function. */
            BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()));

            /** When concatenating strings read from the remote server,
             * we get some performance improvement by using the StringBuilder class. */
            StringBuilder data = new StringBuilder();
            String line;

            /** Keep reading from the remote server until we get a null
             * which indicates the connection has been closed. */
            do {
